...
class WobblerVoice {
  constructor(ctx, masterIn, id) {
    ...
    this._distNode = ctx.createWaveShaper(); this._distNode.oversample = '4x';
  ...
  _tweNoteOn(velocity) {
    ...
    if (ss.depth > 0 && ss._enabled !== false) {
      const fireStrike = () => {
        ...
        this._tweStrikeOsc = strikeOsc; this._tweStrikeGain = gain;
      };
  ...
  noteOn(freq, velocity = 0.65) {
    ...
    const isRetrigger = this.envGain.gain.value > 0.01;

    this._midiFreq = freq;
    const f = freq * Math.pow(2, p.osc.pitch / 12);
    const vol = p.osc.volume * velocity;

    // Mute all osc outputs, then activate only the current waveform mode
    ...
    fc.cancelScheduledValues(now); fc.setValueAtTime(base, now);
  ...
  noteOff() {
    ...
    for (let i = 0; i < this.lfoNodes.length; i++) {
      ...
      if (lp.envR > 0.005 && lp.depth > 0) {
        lfo.gain.gain.cancelScheduledValues(now);
        lfo.gain.gain.setValueAtTime(lfo.gain.gain.value, now);
        lfo.gain.gain.linearRampToValueAtTime(0, now + Math.max(lp.envR, rel));
      }
    }
  }
  ...
  set(section, key, val) {
    ...
    if (section === 'dist') {
      this.p.dist[key] = val;
      if (key === 'form' || key === 'drive') this._distNode.curve = this._distCurve(this.p.dist.form, this.p.dist.drive);
